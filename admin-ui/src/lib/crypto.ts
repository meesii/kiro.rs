...
/**
 * SHA256 哈希（优先 Web Crypto；HTTP 等非安全上下文无 crypto.subtle 时走纯 JS）
 */
async function sha256_hex(input: string): Promise<string> {
    ...
    return sha256_hex_sync(data);
}

function sha256_hex_sync(data: Uint8Array): string {
    ...
    const bit_len = data.length * 8;
    const pad_len = ((data.length + 9 + 63) >> 6) << 6;
    const padded = new Uint8Array(pad_len);
    padded.set(data);
    padded[data.length] = 0x80;
    const view = new DataView(padded.buffer);
    view.setUint32(pad_len - 4, bit_len, false);

    const w = new Uint32Array(64);
    for (let offset = 0; offset < pad_len; offset += 64) {
        for (let i = 0; i < 16; i++) {
